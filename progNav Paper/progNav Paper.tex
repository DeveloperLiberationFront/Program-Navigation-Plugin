...
While working on Task 2 with \toolName~ P6 attempted to utilized Eclipse's built-in back buttons to backtrack, but still had difficulty reorienting himself. 
...
Participants successfully completed simple navigation tasks with \toolName, perhaps due to its minimalistic interface.
...
Many existing tools support systematic exploration through the use of secondary views containing either hierarchically structured lists of methods or call graph visualizations.
In keeping with \toolName's minimalistic design and trying to preserve \toolName's \textit{Low Barriers to Invocation}, 
Admittedly, \toolName's minimalistic interface omits many features that could help developers  
