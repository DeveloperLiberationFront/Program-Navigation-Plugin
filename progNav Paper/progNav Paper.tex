...
The current state-of-the-art tools have been shown to enable effective navigation, but achieve gains by leaning heavily on visualizations and cumbersome interface widgets.
...
Our tool enables developers to simultaneously navigate control flow and data flow, upstream and downstream, within the Eclipse Integrated Development Environment (IDE).
Based on a preliminary evaluation with 8 programmers, our tool seems to succeed when call graphs contain relatively few branches, but seems strained by complex program structures. 
...
Deduced it was our tool. Due to social desirability bias. Study materials do not indicate we created \toolName~. Likely inflated positive responses on the PSSUQ . Look at relative responses rather than absolute values 
...
Based on our preliminary evaluation, \toolName~ seems most effective when developers wish to navigate program structures with few branches.\footnote{This work is supported by NSF grant number 131832}
